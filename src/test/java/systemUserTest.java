...
public class systemUserTest implements DbSystemUser {
   ...
   /*   public SystemUser(String firstName, String lastName, String email, String company, String roles, String phoneNumber) {
    ...
    public systemUserTest(String firstName, String lastName, String email, String company, String roles, String phoneNumber) {
        this.firstName = firstName;
        this.lastName = lastName;
        this.email = email;
        this.company = company;
    ...
    public static List<systemUserTest> all () {
        String sql = "SELECT * FROM systemUser";
        try (Connection con = DbSystemUsers.sql2o.open()) {
            return con.createQuery(sql).executeAndFetch(systemUserTest.class);
        }
    }
    public static systemUserTest find (int userId){
        try (Connection con = DbSystemUsers.sql2o.open()) {
            String sql = "SELECT * FROM systemUser where UserId = :Id";
            return con.createQuery(sql)
                    .addParameter("userId", userId)
                    .executeAndFetchFirst(systemUserTest.class);
        }
    }
